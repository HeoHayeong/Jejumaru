...
<c:choose>
	<c:when test="${empty q_list || fn:length(q_list) == 0 }">
		<script>
...
<ul class="nav navbar-nav ml-auto">
    ...
    <section>
        ...
        <form name = "frm" action="q_updateOk.do" method="post" onsubmit="return chkSubmit()">
        <!-- hidden 타입으로 qno 넘겨줘야한다! -->
        <input type="hidden" name="qno" value="${q_list[0].qno }"/>
        <div class="noticeTable">
            <table>
                <tr >
                    <td style="text-align: left; color: gray; font-size: 14px;">${q_list[0].qmname }</td>                
                    <td style="text-align: right;  color: gray; font-size: 14px;">${q_list[0].qregdate }</td>
                </tr>
                <tr>
                    <th colspan="2">
                    	<input type = "text" name = "qtitle" value="${q_list[0].qtitle} "/>
                    </th>
                </tr>
                <tr> 
                    <td colspan="2" class="contentBox" style="text-align: left;">
                    	<textarea name="qcontent" id="updateEdt">${q_list[0].qcontent}</textarea>
                        <script>
